...
gcloud api-gateway api-configs create gcp-sow-api-config --api=$API_NAME --openapi-spec=$CONFIG_FILE --project=$PROJECT_ID

#deploy the API config to a gateway, run the below command
gcloud api-gateway gateways create gcp-sow-api-gateway --api=$API_NAME --api-config=gcp-sow-api-config --location=us-east4 --project=$PROJECT_ID
